...
export const Vortex = (props: VortexProps) => {
  ...
  const updateParticle = (i: number, ctx: CanvasRenderingContext2D) => {
    ...
    const n = noise3D((x ?? 0) * xOff, (y ?? 0) * yOff, tick * zOff) * noiseSteps * TAU;
    ...
    const x2 = (x ?? 0) + vx * speed;
    const y2 = (y ?? 0) + vy * speed;
    const radius = particleProps[i8] ?? 0;
    const hue = particleProps[i9] ?? 0;

    drawParticle((x ?? 0), (y ?? 0), (x2 ?? 0), (y2 ?? 0), life, ttl, radius, hue, ctx);
    ...
    if (checkBounds((x ?? 0), (y ?? 0), canvas) || life > ttl) {
      initParticle(i);
    }
  };
